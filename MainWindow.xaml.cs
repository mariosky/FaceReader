...
namespace Microsoft.Samples.Kinect.FaceBasics
{
  ...
  // using MySql.Data.MySqlClient;
    ...
    public class FrameEvent
    {
     ...
     public String kinect_id;
     public Int32  date_time;
     public String happy ;
     public String engaged;
     public String looking_away;
     public UInt16 face_index;
     public UInt16 face_box_left;
     public UInt16 face_box_top;
     public UInt16 face_box_right;
     public UInt16 face_box_bottom;
    }
      ...
      //private FaceBasics.MySQL conn = new FaceBasics.MySQL( new MySqlConnection("persistsecurityinfo=True;server=localhost;user=root;password=masterkey;database=facedb"));
        ...
        private void Reader_BodyFrameArrived(object sender, BodyFrameArrivedEventArgs e)
        {
            using (var bodyFrame = e.FrameReference.AcquireFrame())
            {
                if (bodyFrame != null)
                {
                    if (bodyFrame.RelativeTime.Seconds != this.currentSecond)
                    {
                        this.currentSecond = bodyFrame.RelativeTime.Seconds;
        ...
        private void DrawFaceFrameResults(int faceIndex, FaceFrameResult faceResult, DrawingContext drawingContext)
        {
            ...
            command.CommandText = @"INSERT INTO face_frame (
       		                                    kinect_id,
    	                                        date_time,
    	                                        happy,
    	                                        engaged,
            ...
            command.Parameters.Add("@kinect_id", MySqlDbType.VarChar,64).Value = this.kinectSensor.UniqueKinectId;
           
            command.Parameters.Add("@date_time", MySqlDbType.DateTime).Value = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            command.Parameters.Add("@happy", MySqlDbType.VarChar,10).Value = faceResult.FaceProperties[FaceProperty.Happy].ToString();
            command.Parameters.Add("@engaged", MySqlDbType.VarChar, 10).Value = faceResult.FaceProperties[FaceProperty.Engaged].ToString();
            command.Parameters.Add("@looking_away", MySqlDbType.VarChar,10).Value = faceResult.FaceProperties[FaceProperty.LookingAway].ToString();

            command.Parameters.Add("@face_index", MySqlDbType.UInt16).Value = faceIndex;
            command.Parameters.Add("@face_box_left", MySqlDbType.UInt16).Value = faceBoxSource.Left;
            command.Parameters.Add("@face_box_top", MySqlDbType.UInt16).Value = faceBoxSource.Top;
            command.Parameters.Add("@face_box_right", MySqlDbType.UInt16).Value = faceBoxSource.Right;
            command.Parameters.Add("@face_box_bottom", MySqlDbType.UInt16).Value = faceBoxSource.Bottom;
            ...
            */
            ...
            frame.looking_away = faceResult.FaceProperties[FaceProperty.LookingAway].ToString();
            frame.face_index = (UInt16) faceIndex;
            frame.face_box_left = (UInt16) faceBoxSource.Left;
            frame.face_box_top = (UInt16) faceBoxSource.Top;
            frame.face_box_right = (UInt16) faceBoxSource.Right;
            frame.face_box_bottom = (UInt16) faceBoxSource.Bottom;

            var serializer = new JavaScriptSerializer();
            String frame_json = serializer.Serialize(frame);

            IDatabase db = this.redis.GetDatabase();

            String frame_id = String.Format("{0}:{1}:{2}", frame.kinect_id, frame.date_time, frame.face_index);

            //db.StringSet("clave", frame_json);
            //Trace.WriteLine(db.StringGet("clave"));

            var tran = db.CreateTransaction();
            var set =tran.StringSetAsync(frame_id, frame_json);
            var zadd = tran.SortedSetAddAsync(frame.kinect_id,frame_id, frame.date_time);
            tran.ExecuteAsync();
        ...
        private void btnPlay_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (this.bodyFrameReader.IsPaused)
            {
                this.bodyFrameReader.IsPaused = false;
